...
# Actions
from gotoWaypoint import gotoWaypoint
from aproximateToSomebody import aproximateToSomebody
from introMyself import introMyself
from graspObject import graspObject
from placeObject import placeObject

actions = [gotoWaypoint, aproximateToSomebody, introMyself, graspObject, placeObject]
...
def find_action_implementation(action_name):
    ...
    try:
        ...
        for act in actions:
            if issubclass(action_class, act):
                return action_class
            else:
                pass
                # rospy.logwarn("class " + action_class + " must inherit")
                # return None
        rospy.logwarn("class " + str(action_class) + " must inherit")
        return None
pnpas = None
